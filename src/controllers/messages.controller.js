...
export const uploadMessageWithFiles = async (req, res) => {
  try {
    ...
    for (const thumbnail of fileThumbs) {
      ...
      if (uploadedFile) {
        message = await messagesServices.editFileThumbnail(message.id, {
    ...
    for (const mappedFile of mappedFileList) {
      ...
      if (uploadedFile) {
        message = await messagesServices.editFileUrl(message.id, {
          fileId: messageFileId,
      ...
      for (const thumbnail of thumbList) {
        ...
        if (uploadedFile) {
          message = await messagesServices.editFileThumbList(message.id, {
            fileId: messageFileId,
      ...
      message = await messagesServices.editFileStatus(message.id, {
        fileId: messageFileId,
        status: "done",
      });
      socketSpace.emit(SocketEvent.ON_EDITED_MESSAGE, message);
